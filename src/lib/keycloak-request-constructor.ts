...
export const buildLoginRequestUrl = async ({ hint }: { hint : string }) => {
  return `${env.LW_SERVER_URL}${authPath}?client_id=${env.KC_SPRING_CLIENT}&redirect_uri=${env.NEXT_URL}${loginRedirectUri}&response_type=code&scope=openid&state=${new Date().getTime()}&kc_idp_hint=${hint}&prompt=login`
}
...
export const buildLogoutRequestUrl = async () => {
  ...
  return `${env.LW_SERVER_URL}${logoutPath}?id_token_hint=${id_token}&post_logout_redirect_uri=${logout}`
}

export const buildRefreshBody = async () => {
  const refresh_token = (await getToken({ token: 'refresh_token' }))?.sub
  return `grant_type=refresh_token&client_id=${env.KC_SPRING_CLIENT}&client_secret=${env.KC_SPRING_SECRET}&refresh_token=${refresh_token}`
}
